        ...
        function checkFaceVisibility(landmarks) {
            ...
            const margin = 20; // Pixels margin from canvas edges
        ...
        async function predictWebcam() {
            ...
            if (results.faceLandmarks && results.faceLandmarks.length > 0) {
                ...
                if (isFaceVisible) {
                    faceVisibilityStatus.className = 'face-visibility-status face-visible';
                    faceVisibilityStatus.textContent = '✓ Face is properly positioned';
                } else {
                    faceVisibilityStatus.className = 'face-visibility-status face-not-visible';
                    faceVisibilityStatus.textContent = '⚠ Position your face fully within the frame';
                }

                if (currentDistance > 50) {
                     faceCameraVisibilityDistanceStatus.className = 'face-visibility-status face-not-visible';
                     faceCameraVisibilityDistanceStatus.textContent = '⚠ Position your face closer to the camera';
                } else{
                    faceCameraVisibilityDistanceStatus.className = 'face-visibility-status face-not-visible';
                     faceCameraVisibilityDistanceStatus.textContent = '✓ Face is properly positioned near Camera.';
                }
                


                // Draw face boundary landmarks for debugging (optional)
                if (isFaceVisible) {
                    canvasCtx.fillStyle = "blue";
                    for (const pointIndex of FACE_BOUNDARY_LANDMARKS) {
                        if (pointIndex < results.faceLandmarks[0].length) {
                            const point = results.faceLandmarks[0][pointIndex];
                            const x = point.x * canvasElement.width;
                ...
                isFaceVisible = false;
                faceVisibilityStatus.className = 'face-visibility-status face-not-visible';
                faceVisibilityStatus.textContent = '⚠ No face detected';
            }
        ...
        function checkChallenge(blendShapes, faceLandmarks) {
            ...
            if (!isFaceVisible || currentDistance > 50) {
                challengeStartTime = 0;
                // Update progress indicator to show face positioning needed
                progressIndicator.style.background = 'conic-gradient(#ffc107 0deg, #e9ecef 0deg)';
                return;
            }
